    ...
    <header className="no-print mb-4 rounded-lg border border-slate-300 bg-white p-3 shadow-sm dark:border-slate-600 dark:bg-slate-800 sm:p-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          ...
          title="Вийти"
          className="shrink-0 rounded-lg bg-rose-600 px-3 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-rose-700 hover:shadow-md sm:px-4"
        >
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-slate-200 pt-3 dark:border-slate-700">
        <div className="flex min-w-0 items-center gap-3">
          ...
          {criticalRawCount > 0 && (
            <span className="shrink-0 rounded-full bg-red-100 px-2.5 py-1 text-xs font-semibold text-red-700 dark:bg-red-950/40 dark:text-red-200">
              ⚠ {criticalRawCount}
            </span>
          )}
        </div>
        <button
          type="button"
          onClick={toggleTheme}
          className="shrink-0 rounded-md border border-slate-300 bg-white px-3 py-1.5 text-xs text-slate-700 hover:bg-slate-50 dark:border-slate-500 dark:bg-slate-700 dark:text-slate-100 dark:hover:bg-slate-600 sm:text-sm"
        >
